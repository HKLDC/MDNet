...
class MultimodalLateFusion(nn.Module):
    def __init__(self, feat_dim=128, num_views=4, num_classes=2, tau=1.0, gamma=1.0, use_norm=True, fusion_type='adaptive'):
        super().__init__()
    ...
    def forward(self, fea_list, logits_list):
        combined_feats = torch.cat([f.detach() for f in fea_list], dim=-1)
        alpha = self.gate(combined_feats)
        weights = F.softmax(alpha, dim=-1)

        weighted_feats = torch.stack(fea_list, dim=1)
    ...
    def compute_loss(self, logits_list, alpha):
        ...
        target_weights = F.softmax(-torch.stack(view_entropy, dim=1) / self.gamma, dim=-1).detach()
