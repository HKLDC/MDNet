...
SPLIT_DIR = {
    'temporal': 'data-split',
    '5fold': 'data-split-5fold',
}


def get_split_filenames(split, fold):
    if split == 'temporal':
        return {
            'train': 'vid_time3_train.txt',
            'val': 'vid_time3_val.txt',
            'test': 'vid_time3_test.txt',
        }
    elif split == '5fold':
        return {
            'train': 'fold_{}_train.txt'.format(fold),
            'val': 'fold_{}_val.txt'.format(fold),
            'test': 'fold_{}_test.txt'.format(fold),
        }
    else:
        raise ValueError("未知划分方式: {} (可选 'temporal' / '5fold')".format(split))


class BSVFNDDataset(Dataset):
    def __init__(self, path_vid, token, dataset, split='temporal', datamode='title+ocr'):
        ...
        if self.dataset == 'fakesv':
            ...
            with open(os.path.join('data/FakeSV', split_dir, path_vid), "r") as fr:
            ...
            with open(os.path.join('data/FakeTT', split_dir, path_vid), "r") as fr:
                for line in fr.readlines():
                    self.vid.append(line.strip())
