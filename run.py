...
class Run():
    ...
    def get_model(self):
        if self.model_param == 'MDNet':
            self.model = MDNet(dataset=self.dataset,model_path=self.save_param_dir,module_deep=2,dropout=self.dropout)
        else:
            self.model = ModalityModel(dataset=self.dataset,modality=self.model_param,module_deep=2,dropout=self.dropout)
        return self.model

    def main(self):
        self.model = self.get_model()
        dataloaders = self.get_dataloader_temporal()
        trainer = Trainer(model=self.model, device = self.device, lr = self.lr, dataloaders = dataloaders, epoches = self.epoches, dropout = self.dropout, weight_decay = self.weight_decay, epoch_stop = self.epoch_stop, save_param_path = self.save_param_dir+self.dataset+"/"+self.model_param+"/", writer = SummaryWriter(self.path_tensorboard))
